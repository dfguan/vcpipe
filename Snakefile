...
rule bwa_map:
	...
	threads: 4
	shell:
		"""bsub -M4000 -n {threads} -R"span[hosts=1] select[mem>4000] rusage[mem=4000]" -J{output.lsf_j} -o{output.lsf_o} -e {output.lsf_e} "bwa mem -t{threads} {input.idx} {input.i1} {input.i2} | samtools view -h -b - | samtools sort - -o {output.srtbam}" && samtools index {output.srtbam}"""
rule markdups:
	input: 
