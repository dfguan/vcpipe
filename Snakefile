...
rule bwa_map:
	...
	threads: 4
	shell:
		""" bsub -M4000 -n {threads} -R"span[hosts=1] select[mem>4000] rusage[mem=4000]" -J{output.lsf_j} -o{output.lsf_o} -e {output.lsf_e} "bwa mem -t{threads} {ref} {input.i1} {input.i2} | samtools view -h -b - | samtools sort - -o {output.srtbam}" """
...
rule hapcall: # haplotyecaller
	...
	shell:
		""" bsub -M4000 -R"select[mem>4000] rusage[mem=4000]" -J{output.lsf_j} -o{output.lsf_o} -e{output.lsf_e} "java -jar -Xmx4G {gatk_pth} HaplotypeCaller  -I{input.srtbam} -R {ref} -ERC GVCF -G StandardAnnotation -GAS_StandardAnnotation -O {output.ogvcf}" """
...
rule cmbgvcfs:
	...
	shell:
		"""bsub -M4000 -R"select[mem>4000] rusage[mem=4000]" -J{output.lsf_j} -o{output.lsf_o} -e{output.lsf_e} "java -jar -Xmx4G {gatk_pth} CombineGVCFs {params} -R {ref} -O {output.o_gvcfs}" """
rule genotypevcf:
	input:
		i_gvcfs = "cmb.g.vcf",
	output:
		...
		lsf_o = "gnv.o"
	shell:	
		"""bsub -M4000 -R"select[mem>4000] rusage[mem=4000]" -J{output.lsf_j} -o{output.lsf_o} -e{output.lsf_e} "java -jar -Xmx4G {gatk_pth} GenotypeGVCFs -R {ref} -V {input.i_gvcfs} -O {output.o_vcf}" """	
rule fltv: 
	input:
	...
	shell:
		""" bsub -M4000 -R"select[mem>4000] rusage[mem=4000]" -J{output.lsf_j} -o{output.lsf_o} -e{output.lsf_e} "java -jar -Xmx4G {gatk_pth} VariantFiltration -R {ref} -V {input.i_vcf} -filter "QD < 2.0 || FS > 60.0 || MQ < 40.0 || MQRankSum < -12.5 || ReadPosRankSum < -8.0" --filter-name "basic_filtering" -O {f_out}" """ 
