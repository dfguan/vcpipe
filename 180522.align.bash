...
align() {
	...
	bsub -M4000 -n4 -R"span[hosts=1] select[mem>4000] rusage[mem=4000]" -Jalign_"$pre_fn" -oalign_"$pre_fn".o -ealign_"$pre_fn".e "bwa mem -R '@RG\tID:${pre_fn}\tSM:${pre_fn}' -t4 ${idxbase} ${pre_fn}_1.fq ${pre_fn}_2.fq | samtools view -h -b - | samtools sort - -O BAM -o ${pre_fn}.srt.bam && rm -f ${pre_fn}_[12].fq" & 
	# gatk recalibrate or maybe group bams first what I am concerned is the fish genomes can be different	
}
if [ "$#" -lt 2 ]
then
	echo align REF SEQ_DIR
else
	ref=$(readlink -f $1)
